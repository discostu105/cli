...
namespace Microsoft.DotNet.ProjectModel.Compilation
{
    public class LibraryExporter
    {
        ...
        private LibraryExport ExportPackage(PackageDescription package)
        {
            ...
            builder.WithNativeLibraries(PopulateAssets(package, package.Target.NativeLibraries));
            builder.WithRuntimeAssemblies(PopulateAssets(package, package.Target.RuntimeAssemblies));
            builder.WithCompilationAssemblies(PopulateAssets(package, package.Target.CompileTimeAssemblies));
            builder.WithSourceReferences(GetSharedSources(package));
            builder.WithAnalyzerReference(GetAnalyzerReferences(package));

            if (package.Target.ContentFiles.Any())
            {
                ...
                var languageGroups = package.Target.ContentFiles.GroupBy(file => file.CodeLanguage);
